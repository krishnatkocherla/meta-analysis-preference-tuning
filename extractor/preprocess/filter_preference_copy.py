...
for abbr, full_name_tuple in PREFERENCE_ALGOS.items():
    ...
    patterns = re.compile(
        rf"""
        (?!.*\\cite[^{{}}]*{re.escape(full_name)}[^{{}}]*}})  # Block citations containing full name
        (?<!\w)(?<!-)                                      # No preceding word/hyphen  
        \b{re.escape(full_name)}\b                         # Full name as standalone words
        (?!\w)(?!-)                                       # No following word/hyphen
        """, 
